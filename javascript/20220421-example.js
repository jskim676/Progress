...
// 자료 구조
// 변수를 작명할때 "데이터타입" 을 살짝 표시해주는 버릇을 가지면 -> 애둘러서 찾을 필요가 없다.
// "nuri 는 변수는 foodObject 라는 객체의 무언가는 가져다 쓰겠지" -> 예상할 수 있게 된다.
...
// // 변수 stringSum 이라는 녀석은 변수 nuri 라는 변수 와 text 라는 변수를 앞뒤로 합쳤다. 를 stringSum 변수에 대입했다.
// console.log(stringSum);


let pokemon = ["이상해씨", "이상해풀", "이상해꽃", "파이리", "리자드", "리자몽"];
// 배열에는 원소 (요소 element), length 값 6인 데이터를 -> 변수 pokemon 대입했다.
